...
const Header = ({isLarge , change } : any)=>{
  ...
  useEffect(()=>{
    ...
    <div className="nav-right flex justify-center items-center">
        ...
        <button className="flex items-center justify-center" onClick={()=>{setSearch((search)=>{return !search})}}><FaMagnifyingGlass className="w-[25px] h-[25px] font-bold inline text-white md:hidden mr-8 search"/></button>

        <motion.div transition={{layout:{duration : 1 }}} layout className="usermenu relative">

          <button className="profile-section" onClick={()=>{userBtn()}}>
          ...
          <motion.div initial={{opacity : 0}} animate={{opacity:1}} className={`user-actions text-black absolute shadow-lg translate-x-[-100%] w-max min-w-[250px] rounded-md py-2 px-[1rem] bg-white border flex flex-col transition-all dark:bg-[#282828] dark:shadow-[rgb(255,255,255,0.1)]`}> 
           ...
           <button onClick={()=>{signOut()}} className="p-[5%] relative w-full flex rounded-[8px] dark:text-white items-center hover:bg-[rgb(0,0,0,0.1)] bg-[#ff00009f] dark:bg-[#ff00008a] dark:hover:bg-[rgb(255,255,255,0.1)] logoutbtn" > 
           <div className="w-[1.3rem] m-0 mr-[5%]">
